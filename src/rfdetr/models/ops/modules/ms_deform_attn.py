...
class MSDeformAttn(nn.Module):
    ...
    def export(self) -> None:
        """Switch module to export mode for ``torch.export`` / CoreML compatibility.
    ...
    def forward(
        ...
        Raises:
            RuntimeError: If ``self._export`` is True and ``input_spatial_shapes_hw`` is
                ...
                ``torch.export``).
            ValueError: If the last dimension of ``reference_points`` is not 2 or 4, or (in
                export mode) if the level dimension is neither 1 nor ``n_levels``.
        """
        ...
        if self._export:
            # Export path: build sampling_locations at rank 5 by merging (n_levels, n_points) ->
            # n_levels*n_points, so no tensor exceeds rank 5. CoreML's MIL backend rejects rank-6
            # tensors; XNNPACK is unaffected. The merged layout is bit-identical to the rank-6 path
            # below (offset_normalizer / reference_points are repeat_interleaved over n_points to
            # match the [n_levels, n_points] flattening order). The core consumes the rank-5 form
            # (detected by ndim). Only reached in export mode, so eager train/inference is unchanged.
            #
            # Decoder export mode passes reference_points with level dim 1 (shared across levels);
            # the rank-6 path broadcasts via None dims. Expand here before repeat_interleave so the
            # merged axis length is n_levels*n_points, not n_points.
            ...
            if n_ref_levels == 1:
                ...
                raise ValueError(
                    f"reference_points level dim must be 1 or n_levels={self.n_levels}, "
                    f"got {n_ref_levels}"
                )
            if reference_points.shape[-1] == 2:
                offset_normalizer = torch.stack([input_spatial_shapes[..., 1], input_spatial_shapes[..., 0]], -1)
                offset_normalizer = offset_normalizer.repeat_interleave(self.n_points, dim=0)
                ref = reference_points.repeat_interleave(self.n_points, dim=2)
                sampling_locations = (
